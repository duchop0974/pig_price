...
-- Kế hoạch bán (BM02) — do Phòng bán hàng tạo, "nhặt" một phần/toàn bộ số
-- lượng từ MỘT kế hoạch trại (sale_plans) đã duyệt, gán giá chào bán. 1 kế
-- hoạch trại có thể sinh nhiều kế hoạch bán (chia nhỏ lô để bán cho nhiều
-- khách ở nhiều thời điểm). pig_type_id KHÔNG lặp lại ở đây — lấy qua JOIN
-- sale_plan_id (allocation chỉ nhặt đúng loại heo trại đã đăng ký).
...
CREATE INDEX IF NOT EXISTS idx_sale_allocations_status ON sale_allocations(status);
CREATE INDEX IF NOT EXISTS idx_sale_allocations_customer_id ON sale_allocations(customer_id);
-- Index UNIQUE cho plan_code tạo trong _migrate() (cùng lý do idx_sale_plans_plan_code
-- — bảng này có thể được tách ra từ dữ liệu cũ nên tạo sau khi chắc chắn hợp lệ).
